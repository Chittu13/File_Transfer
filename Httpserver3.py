    ...
    def do_POST(self):
        ...
        if "file" in form:
            ...
            if filename:
                ...
                print(f"[+] File uploaded via Web UI: {filename}")
            else:
...
def get_port():
    """Get port from CLI args or ask user."""
    if len(sys.argv) > 1:
        try:
        ...
        user_input = input(f"Enter port to use (default {DEFAULT_PORT}): ").strip()
...
if __name__ == '__main__':
    ...
    # Allow socket reuse (fixes "address already in use" issues after restart)
